...
const WIDGET_FIELD_SPEC: Record<WidgetKind, string> = {
  ...
  animation:
    `{ "kind":"animation", "frames": [ { "id": string, "caption": string, "latex"?: string } ], "motion"?: { "xExpression": string, "yExpression": string, "zExpression"?: string, "tDomain": [number,number], "trace"?: boolean, "guideXExpression"?: string, "guideYExpression"?: string }, "durationMs"?: number, "loop"?: boolean, "predictPrompt"?: string, "respond"?: RespondSpec, "checkpoints"?: [ { "id": string, "at": number, "prompt": string, "options"?: [ { "id": string, "label": string, "correct"?: boolean } ], "acceptedAnswers"?: string[], "rationale"?: string } ], "controls"?: { "scrub"?: boolean, "step"?: boolean, "speed"?: boolean, "replay"?: boolean }, "linkedRepresentations"?: [ { "id": string, "representation": "graph"|"table"|"equation"|"number_line"|"diagram", "label": string, "tracks": string } ], "reconcilePrompt"?: string, "reconstructPrompt"?: string } — 1–12 frames; motion expressions are arithmetic in "t" (add "zExpression" for motion on a 3D graph; the surface renders axes on 2D scenes and an isometric view on 3D ones, so a graph-bound motion is always drawn on its graph — and when the motion travels over a curve, put that curve in guideX/guideY so it renders solidly); 1–6 checkpoints with "at" in 0..1 strictly increasing, and multiple-choice checkpoints need exactly one correct option; "predictPrompt" REQUIRES "respond"; "reconcilePrompt" REQUIRES "predictPrompt".`,
  comparison:
