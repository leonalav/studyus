  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        ...
        streamingSupported: false,
        capabilities: defaultCapabilities(),
        error: `Endpoint returned HTTP ${res.status}: ${errText.slice(0, 150)}`,
      };
    }
...
export async function getSanitizedSettings(): Promise<{
  ...
  return {
    roles: bindings,
    endpoints: [
      { id: "studyus-default", label: "Studyus (default)", provider: "studyus", baseUrl: "https://api.studyus.ai/v1", modelId: "studyus-tutor-3", active: true },
    ],
  };
}
