...
describe("Assessment & Numeric Grader Engine", () => {
  beforeEach(async () => {
    // initialize db + author the physics form/attempt fixtures the
    // state-machine tests below exercise. Idempotent (guarded by a count
    // check), so re-running the suite does not violate primary keys.
