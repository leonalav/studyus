...
import type { RoadmapWidget, WidgetIntent, WidgetKind, WidgetState } from "./widgets/types";
...
// (Cap retired in Phase 1 cleanup; curriculum stores chunks whole and the
//  prompt builder applies its own truncation, so this constant no longer
//  participates in any path. Kept historically in commit history.)
 ...
 */
// (Nudge text retired in Phase 1 cleanup: the policed-turn repair now
//  synthesises the missing widget directly rather than asking the learner to
//  type a continue command. The string is preserved here as a comment so the
...
export function enforceTutorToolPolicy(
  ...
  const boardOps = turn.boardOps.flatMap<BoardOp>((op) => {
    switch (op.op) {
      ...
      // Defensive pass-through for any future `BoardOp` kind added without a
      // policy decision: keep the op rather than dropping it, since the
      // existing tool budget already bound above decides whether the board
...
function validateBoardBlockSpec(value: unknown, path: string, errors: string[], effortConfig?: EffortConstraints): BoardBlockSpec | null {
...
function validateBoardOp(value: unknown, path: string, errors: string[], effortConfig?: EffortConstraints): BoardOp | null {
  ...
  switch (op) {
    ...
    case "insert_after": {
      const target = validateBoardTarget(rec, path, errors);
      const block = validateBoardBlockSpec(rec.block, `${path}.block`, errors, effortConfig);
    ...
    case "spawn_thread": {
      ...
      rawBlocks.slice(0, MAX_THREAD_INITIAL_BLOCKS).forEach((entry, index) => {
        const block = validateBoardBlockSpec(entry, `${path}.initial_blocks[${index}]`, errors, effortConfig);
    ...
    case "spawn_prerequisite_thread": {
      ...
      rawBlocks.forEach((entry, index) => {
        const block = validateBoardBlockSpec(entry, `${path}.teaching_blocks[${index}]`, errors, effortConfig);
...
export interface LearnerState {
  ...
  /** Learner-edited plan submitted via a plan widget. Present only on the
   *  `plan_start` turn that follows the agreement click; the harness persists
   *  it as a binding contract and surfaces it in the contract reminder so the
   *  tutor teaches to the learner's chosen route, not the proposed one. */
  bindingPlan?: { heading: string; steps: { id: string; label: string; details?: string[] }[] };
}
...
export async function askTutorTurn(req: TutorTurnRequest): Promise<StructuredCallResult<TutorTurn>> {
  ...
  // Persist a learner-edited plan as a binding TurnContract. The agreement
  // click carries `learner.bindingPlan` from the widget state; without it the
  // tutor keeps its proposed plan, which is the right default for a learner
  // who agreed verbatim. We persist best-effort: a contract write that fails
  // is logged but does not cost the learner their first teaching turn — the
  // roadmap will simply have to honour the proposed plan rather than a
  // binding learner route.
  //
  // The plan is encoded as a `goal` commitment per step (one row per phase)
  // so the contract reminder surfaces each step individually and the tutor
  // can tell which step it is teaching.
  if (turnKind === "plan_start" && learner.bindingPlan) {
    try {
      const { saveContract, listAllContracts } = await import("./contracts/store");
      const existing = await listAllContracts(learnerId);
      const lineage = existing.filter((c) => c.sessionId === sessionId);
      const nextRevision = lineage.length === 0
        ? 1
        : Math.max(...lineage.map((c) => c.revision)) + 1;
      const stepLines = learner.bindingPlan.steps
        .map((s, i) => {
          const detail = s.details?.length ? ` — ${s.details.join("; ")}` : "";
          return `${i + 1}. ${s.label}${detail}`;
        })
        .join("\n");
      await saveContract({
        contractId: `plan-${sessionId}`,
        ...
        commitments: [
          {
            kind: "goal",
            statement: `Binding learning route for "${learner.bindingPlan.heading}":\n${stepLines}`,
          },
        ],
      ...
      console.warn("[tutor] could not persist binding plan contract", err);
    }
  }
  ...
  try {
    ...
    if (!isPlanningTurn) {
      await rememberTutorDiagnosis(sessionId, result.value.diagnosis, studio, result.value.evidenceRefs, learnerId, skillId);
    }
