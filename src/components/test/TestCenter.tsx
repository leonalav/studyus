  ...
  useEffect(() => {
    (async () => {
      ...
      if (res[0]) {
        ...
        // Fallback default
        const defaultSrc = [
          { id: "c-calc-vol-2", name: "calculus-volume-2_-_WEB.pdf", pageCount: 312 },
          { id: "c-phys-hl", name: "IB Physics HL — Syllabus 2025.pdf", pageCount: 88 },
        ];
        setSources(defaultSrc);
        setSelectedSourceId(defaultSrc[0].id);
      }
    })();
  }, [selectedSourceId]);
    ...
    <div className="mx-auto w-full max-w-[860px] px-5 pt-10 pb-20 select-none">
      ...
      <div className={`mb-6 overflow-hidden rounded-md border border-edge bg-raise ${mode === "final" ? "opacity-60" : ""}`}>
        {nodes.map((section, i) => {
          ...
          const someOn = allSecIds.some((id) => effectivePicked.has(id));
